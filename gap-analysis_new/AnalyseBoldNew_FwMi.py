...
def loadOldGap(OldGap):
    ...
    for da in data[1:]:
        ...
        info[da[2]] = [da[0],da[1],int(da[6]),da[7],da[8]]
        ppg[da[2]] = [int(d) for d in da[3:6]]
        if da[8] != "":
            for s in da[8].split(";"):
                syns[s] = da[2]
    return(info,syns,ppg)

#Load single Progress Report
def loadSingleProg(data,info,syns,proData,Kingdom):
    for da in data:
        da = da.strip(" ").split("\t")
        if len(da) > 6:
...
def loadProgress(ProgFolder,info,syns,Kingdom):
    files = os.listdir(ProgFolder)
    proData = {}
    for f in files:
        data = open(ProgFolder + "/" + f).read().split("\n")
        proData = loadSingleProg(data,info,syns,proData,Kingdom)
    return(proData)

def updateBpData(bpdata,da,rtTerms,rtnew,rtcount,syns):
    da = da.split("\t")
    sp = da[0]
    if sp in syns.keys():
        sp = syns.get(sp)
...
def loadBoldpub(BoldDaset,info,syns,rtTerms):
    ...
    for f in files:
        ...
        if len(data) > 1:
            data = data[1:]
            for da in data:
                bpdata,rtnew,rtcount = updateBpData(bpdata,da,rtTerms,rtnew,rtcount,syns)
    return(bpdata,rtnew,rtcount)
...
def makeOutputGap(info,syns,proData,bpData,outputGap,outputRT):
    outGap = open(outputGap,"w")
    outRT = open(outputRT,"w")

    outGap.write("Taxonomic group\tTaxonomic subgroup\tSpecies\tPublic\tPrivate\tGenBank\tNumber of Countries\tCountries\tSynonyms")
    outRT.write("Taxonomic group\tTaxonomic subgroup\tSpecies\tPublic\tPrivate\tGenBank\tNumber of Countries\tCountries\tSynonyms")
    ...
    for sp in specs:
        ...
        outGap.write("\t{}\t{}\t{}".format(str(inf[2]),inf[3],inf[4]))
        outRT.write("\t{}\t{}\t{}".format(str(inf[2]),inf[3],inf[4]))
...
OldGap = "OldData/FwMi_GapOld.txt"
ProgFolder = "progressReports"
Kingdom = "Animals"
BoldDaset = "rawdata/FwMi"

outputGap = "NewData/FwMi_Gap.txt"
outputRT = "NewData/FwMi_Gap_RT.txt"
outputRTT = "RT/FwMi_RT_Overview.txt"

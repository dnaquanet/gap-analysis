...
def loadOldGap(OldGap):
    ...
    for da in data[1:]:
        ...
        info[da[2]] = [da[0],da[1],da[6]]
        total[da[2]] = [int(d) for d in da[3:6]]
    return(info,total)
    ...
    for da in data:
        ...
        if len(da) > 6:
            if da[2] == "Species" and da[1] == Kingdom:
                if da[0] in info.keys() and da[0] not in proData.keys():
                    spe = da[0]
                    c = 0
                    if da[7] != "":
                        c += int(da[7])
...
def addBoldpub(BoldDaset,info,rtTerms,bpdata,rtnew,rtcount):
    files = os.listdir(BoldDaset)
    for f in files:
        data = open(BoldDaset + "/" + f).read().replace("\r\n","\n").split("\n")
        if len(data) > 1:
...
def makeOutputGap(info,proData,bpData,outputGap,outputRT):
    ...
    outGap.write("Taxonomic group\tTaxonomic subgroup\tSpecies\tPublic\tPrivate\tGenBank\tEcological classification")
    outRT.write("Taxonomic group\tTaxonomic subgroup\tSpecies\tPublic\tPrivate\tGenBank\tEcological classification")
    ...
    for sp in specs:
        ...
        outGap.write("\t".join([str(v) for v in ppg]) + "\t" + inf[2])
        outRT.write("\t".join([str(v) for v in ppgRT]) + "\t" + inf[2])
...
OldGap = "OldData/MaAm_GapOld.txt"
ProgFolder = "progressReports"
Kingdom = "Animals"
BoldDaset = "rawdata/MaEr"
BoldDaset2 = "rawdata/MaAmis"

outputGap = "NewData/MaAm_Gap.txt"
outputRT = "NewData/MaAm_Gap_RT.txt"
outputRTT = "RT/MaAm_RT_Overview.txt"
...
"molecular approach","dna barcode 99.69% match","by dna sequence","dna barcode blast","dna barcode 100% match","bold id engine and genbank","bin taxonomy match and specimen image","molecular identification","molecular: coi","barcoding", "molecular","from dna barcode 99%"]

syns = {}


info,dataO = loadOldGap(OldGap)
proData = loadProgress(ProgFolder,info,Kingdom)
bpData,rtnew,rtcount = loadBoldpub(BoldDaset,info,rtTerms)
bpData,rtnew,rtcount= addBoldpub(BoldDaset2,info,rtTerms,bpData,rtnew,rtcount)
